...
def make_fairytale_prompt(num_pages, transcript):
    return f"""
        ...
        - 전체 동화는 총 {num_pages} 페이지로 구성되어야 해.
        - 동화에는 절대로 사람의 이름이 들어가서는 안돼.
        - "주인공 이름"이 현실 세계의 사람이 이름이 아니라, 동화 속 주인공이라는 걸 알 수 있도록 간단하게 설명하는 문장이 꼭 들어가야 해.
        - 각 페이지는 다음과 같은 필드로 구성된 JSON 객체야:
        - "page": 페이지 번호 (0부터 시작)
